...
按钮命名为「查看」就必须能看，不能只复制。查看弹窗（`#llm-view-modal`）的行为：

1. 打开时**默认打码**（`••••••••••••`），明文不进入 DOM。
2. 「显示 / 隐藏」切换明文；明文放在 `.secret-value` 容器里。
3. 「复制」写入剪贴板；**剪贴板被拒时（非安全上下文、无权限、旧浏览器）
   直接展开明文**让用户长按手动复制——而不是把明文拼进一行状态文字。
4. **30 秒后自动隐藏**：清空保存明文的变量、恢复打码、禁用「显示」；
   关闭弹窗时同样清空变量并 `clearTimeout`。
5. `hint.last4` 明文存储于 `meta`。风险评估：密钥为高熵随机串，泄露末 4 位不构成
   有效攻击面（与 Stripe 等平台的通行做法一致），但**必须写入安全说明**，让使用者知情。
6. 明文不得写入 `localStorage` / `sessionStorage` / URL / `title`。
...
| 长值不写入 DOM 常驻 | 打码是默认态 | 明文只在「显示」后进入 DOM，30 秒后清空 |
