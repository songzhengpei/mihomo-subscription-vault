...
| 约定 | 实现 |
| --- | --- |
| 先在登录页校验，再显示主界面 | `checkSession()` **先**校验会话，通过后才 `showMainScreen()` + 占位。曾试过先渲染骨架，未登录访问会闪一下管理界面，已按反馈回退 |
...
### 7A.3 写路径与外部依赖：把不属于响应的工作挪走

| 操作 | 之前压在响应路径上的工作 | 现在 |
| --- | --- | --- |
| 新建凭据 | 1 次存在性预检读 + 2 次写 | **2 次写**（条件写本身就是冲突检查；meta 写失败时回滚刚写入的密文，保持「缺失」状态不变为「哈希不符」） |
| 轮换密钥 | 读 meta + **读密文取 ETag** + 写密文 + 写 meta | 读 meta + 写密文 + 写 meta（ETag 记录在 meta 里；旧记录无此字段时回退为读一次） |
| 更新订阅 | 发布后**同步**执行版本 prune（一次完整历史扫描 + 删除） | 交给 `ExecutionContext.waitUntil`，响应先返回；无 waitUntil 时（CLI/测试）仍同步等待 |
| WebDAV 推送 | 打包 → **再** MKCOL → PUT（两次外部往返串行） | MKCOL 与打包**并行**，省掉一次外部往返；打包结果不再手工分块拼接 |

无法优化的部分（要如实告知）：**上游订阅拉取**的耗时属于机场服务器；**WebDAV PUT** 的耗时
属于目标网盘与网络。这两项只能通过上述方式把周边自己的往返去掉、让进度更早可见。
...
- **v8（2026-09-11）** 性能轮之二：回退"先渲染骨架"（未登录会闪管理界面）；
  新建/轮换凭据各省一次 R2 读（条件写即冲突检查、ETag 记入 meta）；
  发布后的版本 prune 交给 `waitUntil`；WebDAV 的 MKCOL 与打包并行。
  新增 prune 延迟、创建零读、轮换零读三组守卫。
